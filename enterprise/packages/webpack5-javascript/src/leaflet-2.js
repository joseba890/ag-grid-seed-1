...
function getColor(d, pctlsArray) {
	return d >= pctlsArray[1] ? 'rgb(255,0,0)' : d >= pctlsArray[0] ? 'rgb(255,255,0)' : 'rgb(0,128,0)';
}
...
function addInfoDiv() {
	...
	info.update = function (props) {
		const contents = props
			? `<b>${props.NAME}</b><br>${props.COUNTY} County, NH<br>Tax Rate: ${props.TOTAL.toFixed(2)}<br>Municipal: ${props.MUNI.toFixed(2)}<br>County: ${props.CNTY.toFixed(2)}<br>State Edu.: ${props.STATE_EDU.toFixed(
					2
...
function createMap() {
	...
	map.attributionControl.addAttribution('Tax rate data provided by the NH Department of Revenue Administration');
...
function addLegend() {
	...
	legend.onAdd = () => {
		...
		for (let i = 0; i < levels.length; i++) {
			labels.push(`<i style="border:solid var(--border) thin;background:${colors[i]}"></i> ${levels[i]}`);
		}

		const text = `<div class="bold center" style="margin-bottom:5px;">2024 NH<br>Tax Rates</div>${labels.join('<br>')}`;
...
async function addTileLayers() {
	const styleUrl = 'https://{s}.basemaps.cartocdn.com/{variant}/{z}/{x}/{y}{r}.png';
